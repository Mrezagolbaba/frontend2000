...
export default function ContactUs() {
  ...
  const contactUs = [
    {
      icon: <CuEmail />,
      darkIcon: <DarkCuEmail />,
      info: 'support@arsonex.com',
    },
    {
      icon: <CuPhone />,
      darkIcon: <DarkCuPhone />,
      info: <> 021-92004581 &nbsp; &nbsp; 021-92004581</>,
    },
  ];
  ...
  const locationInfo = [
    {
      ...
      info: 'خیابان ۱۵ خرداد بازار بین الحرمین کوچه شیخ رضا پلاک ۴۶',
    },
    {
      title: 'کد پستی',
      icon: <Email />,
  ...
  return (
    <main className="container">
      ...
      <section
        className={`${styles.contact_us_info_sections} ${styles.flex_co}`}
      >
        <div className={styles.contact_us_box}>
          <ul className={styles.contact_us_location_info}>
            {locationInfo.map((item, index) => (
              <li key={index}>
                <span className={styles.contact_us_location_info_title}>
                  {theme === 'dark' ? item.darkIcon : item.icon} {item.title}
                </span>
                <span className={styles.contact_us_location_info_detail}>
                  {item.info} <Copy label="کپی " />{' '}
                </span>
              </li>
            ))}
          </ul>
          ...
          <div className={styles.contact_us_social_media_holder}>
            <h6 className={styles.contact_us_social_media_title}>
